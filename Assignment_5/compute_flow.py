...
def plot_quiver(flow_elements, t):
    ...
    filename = "./case4/velocity_field/%04d.png" %(t)
...
def init_panels(freestream_vortex_field):
    ''' Initialises/Re-assigns gamma values to 
        the panels on the body
        INPUT - velocity field at the body panel control points
        OUTPUT - panels with appropiate vortex strengths''' 
    panel = []
    #solve for gammas by solving the system of linear equations for no-penetration
...
def compute_flowelement_velocity(flow_elements, panel):
    ...
    if len(flow_elements)!=0:
        flow_element_velocity += n_logn(flow_elements)
    for i in range(len(flow_elements)):
        location = flow_elements[i].current_location
        flow_element_velocity[i] += free_stream.field() + compute_panel_field(panel, location) 
...
def distribute_vortex_blobs(body, panel, flow_elements, slip_velocity):
    ...
    for i in range(body.n_panels):
        ...
        # if abs(slip_velocity[i])-abs(blob_quant*gamma_max)!=0:
        #     remaining_strength = np.sign(gamma_max)*(abs(slip_velocity[i]) - abs(gamma_max)*blob_quant)*body.panel_length
...
def plot_particles(flow_elements, t, file_factor):
    ...
    plt.xlabel(r"X",fontsize=15)
    plt.ylabel(r"Y",fontsize=15)
    filename = "./case4/particles/%04d.png" % int(t/file_factor)
    plt.savefig(filename)
    plt.clf()
...
def solve_flow(flow_elements, panel):
    for t in range(nt):
        ...
        #Compute the velocity at body panel control points 
        ...
        if (t+1)%file_factor==0:
            plot_particles(flow_elements, t+1, file_factor)
...
#initiate Flow elements with empty lists
flow_elements = []
vortex_momentum = []
#initiate free stream
...
#initiate panels with initial vortex sheet strengths
freestream_vortex_field = compute_freestream_vortex_field(flow_elements)
